...
export class GoToActionStore {
	...
	saveTextSizeMetadata(metadata: {
		prefixSizeInPx: number;
		charSizeInPx: number;
	}) {
	...
	goToFromInput() {
		...
		const [line, column] = this.locationInput
			.split(":")
	...
	goTo(lineNumber: number, column = 0) {
		...
		lineNumber = Math.min(
			lineNumber,
			getContainer().currentFileStore.totalLines,
		);

		const numberOfVisibleLines = Math.trunc(
			getContainer().currentInstanceStore.windowInnerHeight / LINE_HEIGHT,
		);
