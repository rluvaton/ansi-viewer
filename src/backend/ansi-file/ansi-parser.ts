...
class Color {
  ...
  css(inverted: boolean) {
    ...
    const rgbName =
      (color.brightness === Code.bright &&
        asBright[color.name as keyof typeof asBright]) ||
      color.name;
 ...
 */
export async function* parseAnsiFromRawSpans(
  rawSpansIterator: AsyncGenerator<Span, void, any>,
): AsyncGenerator<Span, void, any> {
  let color = new Color();
  ...
  for await (const span of rawSpansIterator) {
    ...
    if (span.text !== '') {
      ...
      const newSpan = new Span(
        span.code ? span.code.clone() : undefined,
        span.text,
      );

      newSpan.css = span.css
        ? span.css
        : bold +
          italic +
          underline +
...
export function parseAnsiTransformer() {
  return compose(rawParse, parseAnsiFromRawSpans);
}
