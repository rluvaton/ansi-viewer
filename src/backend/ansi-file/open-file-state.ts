...
function buildHtmlForItems(lineIndex: number, items: LineItem[]): Line {
    ...
    return {lineIndex, __html: `<code contenteditable="false" class="line-number noselect">${lineIndex + 1}</code><pre contenteditable="true" spellcheck="false" data-disable-content-edit data-line class="strip-content-editable-style">${items.map((item) => `<pre ${item.className ? `class="${item.className}"` : ''}>${item.text}</pre>`).join('')}</pre>`}
}
